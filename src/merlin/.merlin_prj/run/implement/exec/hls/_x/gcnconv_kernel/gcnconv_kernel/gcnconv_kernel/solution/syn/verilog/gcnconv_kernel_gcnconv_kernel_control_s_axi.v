...
// ==============================================================
`timescale 1ns/1ps
module gcnconv_kernel_gcnconv_kernel_control_s_axi
#(parameter
...
localparam
    ...
    ADDR_IER                = 6'h08,
    ADDR_ISR                = 6'h0c,
    ADDR_X_DATA_DATA_0      = 6'h10,
    ADDR_X_DATA_DATA_1      = 6'h14,
    ADDR_X_DATA_CTRL        = 6'h18,
    ADDR_WEIGHT_DATA_DATA_0 = 6'h1c,
    ADDR_WEIGHT_DATA_DATA_1 = 6'h20,
    ADDR_WEIGHT_DATA_CTRL   = 6'h24,
    ADDR_EDGE_INDEX_DATA_0  = 6'h28,
    ADDR_EDGE_INDEX_DATA_1  = 6'h2c,
    ADDR_EDGE_INDEX_CTRL    = 6'h30,
    ADDR_RESULT_DATA_DATA_0 = 6'h34,
    ADDR_RESULT_DATA_DATA_1 = 6'h38,
    ADDR_RESULT_DATA_CTRL   = 6'h3c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_edge_index = 'b0;
    reg  [63:0]                   int_result_data = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_EDGE_INDEX_DATA_0: begin
                    rdata <= int_edge_index[31:0];
                end
                ADDR_EDGE_INDEX_DATA_1: begin
                    rdata <= int_edge_index[63:32];
                end
                ADDR_RESULT_DATA_DATA_0: begin
                    rdata <= int_result_data[31:0];
                end
                ADDR_RESULT_DATA_DATA_1: begin
                    rdata <= int_result_data[63:32];
                end
            endcase
        end
    end
...
assign edge_index  = int_edge_index;
assign result_data = int_result_data;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        int_edge_index[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_EDGE_INDEX_DATA_0)
            int_edge_index[31:0] <= (WDATA[31:0] & wmask) | (int_edge_index[31:0] & ~wmask);
    end
end

// int_edge_index[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_edge_index[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_EDGE_INDEX_DATA_1)
            int_edge_index[63:32] <= (WDATA[31:0] & wmask) | (int_edge_index[63:32] & ~wmask);
    end
end

// int_result_data[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_result_data[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_RESULT_DATA_DATA_0)
            int_result_data[31:0] <= (WDATA[31:0] & wmask) | (int_result_data[31:0] & ~wmask);
    end
end

// int_result_data[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_result_data[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_RESULT_DATA_DATA_1)
            int_result_data[63:32] <= (WDATA[31:0] & wmask) | (int_result_data[63:32] & ~wmask);
    end
end
