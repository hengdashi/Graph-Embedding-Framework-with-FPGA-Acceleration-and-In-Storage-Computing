...
typedef struct {
    u32 addr;
    u32 size;
} XGcnconv_kernel_gcnconv_kernel_uio_map;
...
typedef struct {
    ...
    XGcnconv_kernel_gcnconv_kernel_uio_map maps[ MAX_UIO_MAPS ];
} XGcnconv_kernel_gcnconv_kernel_uio_info;

/***************** Variable Definitions **************************************/
static XGcnconv_kernel_gcnconv_kernel_uio_info uio_info;
...
static int uio_info_read_name(XGcnconv_kernel_gcnconv_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGcnconv_kernel_gcnconv_kernel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGcnconv_kernel_gcnconv_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGcnconv_kernel_gcnconv_kernel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGcnconv_kernel_gcnconv_kernel_Initialize(XGcnconv_kernel_gcnconv_kernel *InstancePtr, const char* InstanceName) {
	XGcnconv_kernel_gcnconv_kernel_uio_info *InfoPtr = &uio_info;
