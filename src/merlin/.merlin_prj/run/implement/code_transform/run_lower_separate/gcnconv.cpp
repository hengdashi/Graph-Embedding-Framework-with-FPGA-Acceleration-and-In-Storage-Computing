...
void gcnconv_kernel(
    float x[19717*500],
    float weight[500*16],
    int edge_index[2*(88648 +19717)],
    float result[19717*16]) {
  ...
  matrix_mul_i: for (int i = 0; i < 19717; ++i) {
    // #pragma ACCEL pipeline
    // #pragma ACCEL false_dependence variable=x_mul
    matrix_mul_j: for (int j = 0; j < 16; ++j) {
      float mul = 0;
      // #pragma ACCEL parallel factor=128 reduction=mul
      matrix_mul_k: for (int k = 0; k < 500; ++k) {
  ...
  // #pragma ACCEL pipeline
  assign_index_i: for (int i = 0; i < 19717; ++i) {
  ...
  // #pragma ACCEL pipeline
  assign_weight: for (int i = 0; i < 88648 +19717; ++i) {
    edge_weight[i] = 1;
  }

  // #pragma ACCEL pipeline
  inc_degree: for (int i = 0; i < 88648 +19717; ++i) {
    deg[edge_index[i]] += edge_weight[i];
  }

  // #pragma ACCEL pipeline
  invert: for (int i = 0; i < 19717; ++i) {
    deg_inv_sqrt[i] = 1 / sqrtf(deg[i]);
  }

  // #pragma ACCEL pipeline
  assign_norm: for (int i = 0; i < 88648 +19717; ++i) {
    norm[i] = deg_inv_sqrt[edge_index[i]] * edge_weight[i] * deg_inv_sqrt[edge_index[(88648 +19717) + i]];
  }

  // #pragma ACCEL pipeline
  calc_result_i: for (int i = 0; i < 88648 +19717; ++i) {
    // #pragma ACCEL parallel 
    calc_result_j: for (int j = 0; j < 16; ++j) {
