...
printf("[Merlin Info] Start %s data for %s, data size = %s...\n", "copy in", "x", "9858500 * sizeof(float )");
      
fflush(stdout);
      if (x != 0) {
//         __gcnconv_kernel__x_buffer;
...
printf("[Merlin Info] Finish %s data for %s.\n", "copy in", "x");
...
fflush(stdout);
      if (weight != 0) {
//         __gcnconv_kernel__weight_buffer;
...
fflush(stdout);
      if (edge_index != 0) {
//         __gcnconv_kernel__edge_index_buffer;
...
fflush(stdout);
      if (result != 0) {
//         __gcnconv_kernel__result_buffer;
...
#pragma ACCEL kernel
{
	printf("[Merlin Info] Start set kernel %s argument....\n", "gcnconv_kernel");
	fflush(stdout);
	...
	printf("[Merlin Info] Finish set kernel %s argument....\n", "gcnconv_kernel");
	fflush(stdout);
}
{
	printf("[Merlin Info] Start execute kernel %s....\n", "gcnconv_kernel");
	fflush(stdout);
	size_t __gid[1];
	__gid[0] = 1;
...
}
{
	...
	printf("[Merlin Info] Finish execute kernel %s....\n", "gcnconv_kernel");
	fflush(stdout);
	cl_ulong gcnconv_kernel_start, gcnconv_kernel_end;
	clGetEventProfilingInfo(__event_gcnconv_kernel, CL_PROFILING_COMMAND_START, sizeof(gcnconv_kernel_start), &gcnconv_kernel_start, NULL);
	clGetEventProfilingInfo(__event_gcnconv_kernel, CL_PROFILING_COMMAND_END, sizeof(gcnconv_kernel_end), &gcnconv_kernel_end, NULL);
	printf("[Merlin Info] gcnconv_kernel: Kernel execution time = %0.3fs\n", (gcnconv_kernel_end -gcnconv_kernel_start) * 1e-9);
	fflush(stdout);
}
{
/*
...
__merlin_init_gcnconv_kernel();
  __merlinwrapper_gcnconv_kernel(x,weight,edge_index,result);
  
__merlin_release_gcnconv_kernel();
}
...
fflush(stdout);
    if (x != 0) {
//       __gcnconv_kernel__x_buffer;
//       __event___gcnconv_kernel__x_buffer;
...
fflush(stdout);
    if (weight != 0) {
//       __gcnconv_kernel__weight_buffer;
//       __event___gcnconv_kernel__weight_buffer;
...
fflush(stdout);
    if (edge_index != 0) {
//       __gcnconv_kernel__edge_index_buffer;
//       __event___gcnconv_kernel__edge_index_buffer;
...
fflush(stdout);
    if (result != 0) {
//       __gcnconv_kernel__result_buffer;
//       __event___gcnconv_kernel__result_buffer;
